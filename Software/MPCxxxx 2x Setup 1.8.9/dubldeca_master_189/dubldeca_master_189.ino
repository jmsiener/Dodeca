...
Right Side Dodceca - DUBLDECA layout
...
// #include <C:\Users\Jake Siener\Documents\Arduino\libraries\arduino_midi_library-master\src\MIDI.h>
#include <MIDI.h>
MIDI_CREATE_DEFAULT_INSTANCE();

const uint8_t CHANA = 10;  //set the MIDI channel here! A for drums, B for voice
const uint8_t CHANB = 11;

uint8_t out2pin[] = {23, 0, 22, 25, 20, 6, 21, 5, 9, 4, 10, 3};//output number to actual teensy pin, dont change.
uint8_t whitekeys[] = {0, 4, 8, 6, 0, 0, 10, 0, 9, 11, 0, 0};//non zero keys sent to output number.
//                    {C, #, D, #, E, F, # , G, #, A , #, B}
uint8_t pulses;           //current clock
uint8_t sixteenthnotes;   //current beat
uint8_t thirtysecondths;
uint8_t quartertoggle;    //current frame
uint8_t wholetoggle;
bool playing;
